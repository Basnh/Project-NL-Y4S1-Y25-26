...
public class SSHController {
        ...
        for (String host : hosts) {
            ...
            if (sessionUser != null) {
                Server s = sessionUser.getServers().stream()
                    .filter(server -> server.getIp() != null && server.getIp().equals(ipFinal))
                    .findFirst()
                    .orElse(null);
                if (s != null) {
    ...
    public String saveMultiEditConfig(
        ...
        for (int i = 0; i < ips.size(); i++) {
            ...
            if (sessionUser != null) {
                Server s = sessionUser.getServers().stream()
                    .filter(server -> server.getIp() != null && server.getIp().equals(ip))
                    .findFirst()
                    .orElse(null);
                if (s != null) {
    ...
    private String[] getUserCredentials(String host, HttpSession session) {
        ...
        if (sessionUser != null) {
            Server server = sessionUser.getServers().stream()
                .filter(s -> s.getIp() != null && s.getIp().equals(host))
                .findFirst()
                .orElse(null);
